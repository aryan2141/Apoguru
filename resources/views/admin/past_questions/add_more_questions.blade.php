...
@section('title')
apoGuru - Add More Past Questions
@endsection
                                        ...
                                        <div class="form-line">
                                          <select id="main_category" class="form-control show-tick" name="main_category_id"  required>
                                            ...
                                            @foreach($main_categories as $category)
                                              <option value="{{$category->id}}" {!! $category->id == Session::get('main_cat') ? 'selected':'' !!}>{{$category->name}}</option>
                                            @endforeach
                                          </select>
                                    ...
                                    <div class="form-group form-float">
                                        <div class="form-line">
                                         <select id="parent_sub_category" class="form-control show-tick" name="parent_sub_category_id"  required>
                                            ...
                                            @foreach($parent_sub_categories as $question)
                                            <option value="{{$question->id}}" {!! $question->id == Session::get('parent_cat') ? 'selected':'' !!}>{{$question->name}}</option>
                                            @endforeach
                                          </select> 
                                    ...
                                    <div class="form-group form-float">
                                        <div class="form-line">
                                          <select id="child_sub_category" class="form-control show-tick" name="child_sub_category_id" required>
                                            ...
                                            @foreach($child_sub_categories as $question2)
                                            <option value="{{$question2->id}}" {!! $question2->id == Session::get('child_cat') ? 'selected':'' !!}>{{$question2->name}}</option>
                                            @endforeach
                                          </select> 
                                    ...
                                    <div class="form-group form-float">
                                        <div class="form-line">
                                          <select id="child_sub_category" class="form-control show-tick" name="exam_month" required>
                                            ...
                                            <option {!! 'January' == Session::get('exam_month') ? 'selected':'' !!} value="January">January</option>
                                            <option {!! 'February' == Session::get('exam_month') ? 'selected':'' !!} value="February">February</option>
                                            <option {!! 'March' == Session::get('exam_month') ? 'selected':'' !!} value="March">March</option>
                                            <option {!! 'April' == Session::get('exam_month') ? 'selected':'' !!} value="April">April</option>
                                            <option {!! 'May' == Session::get('exam_month') ? 'selected':'' !!} value="May">May</option>
                                            <option {!! 'June' == Session::get('exam_month') ? 'selected':'' !!} value="June">June</option>
                                            <option {!! 'July' == Session::get('exam_month') ? 'selected':'' !!} value="July">July</option>
                                            <option {!! 'August' == Session::get('exam_month') ? 'selected':'' !!} value="August">August</option>
                                            <option {!! 'September' == Session::get('exam_month') ? 'selected':'' !!}value="September">September</option>
                                            <option {!! 'October' == Session::get('exam_month') ? 'selected':'' !!} value="October">October</option>
                                            <option {!! 'November' == Session::get('exam_month') ? 'selected':'' !!} value="November">November</option>
                                            <option {!! 'December' == Session::get('exam_month') ? 'selected':'' !!} value="December">December</option>
                                          </select>
                                        </div>
                                    </div>
                                </div>
                                    ...
                                    <div class="form-group form-float">
                                        <div class="form-line">
                                          <select id="child_sub_category" class="form-control show-tick" name="exam_year" required>
                                            ...
                                            @for($i=2023; $i>1990; $i--)
                                              <option {!! $i == Session::get('exam_year') ? 'selected':'' !!} value="{{ $i }}">{{ $i }}</option>
                                            @endfor
                                          </select>
                                        </div>
                                    </div>
                                        ...
                                        <div class="form-line">
                                          <select id="question_type_id" class="form-control show-tick" name="question_type_id" required>
                                            ...
                                            @foreach($question_types as $question)
                                              <option {!! $question->id == Session::get('qes_type') ? 'selected':'' !!} value="{{ $question->id }}">{{ $question->name }}</option>
                                            @endforeach
                                          </select>
                                        </div>
                                    </div>
...
$('#parent_sub_category').on('change', function (e) {
  ...
  //localStorage.setItem("parent_sub_category_id",JSON.stringify({"parent_sub_category_id":parent_sub_category_id}));
...
$(document).ready(function () {
  var child_sub_category_id = $('#child_sub_category').val();
  $.ajax({
    type: "POST",
    url: "{{route('admin.get.topic.sub.category.from.child.sub.topic')}}",
    dataType: 'json',
